...
def load_projects():
    ...
    if git_repo:
        git_pull()
        projects = json.loads(find_yml_files(GIT_YML_PATH), encoding='utf-8')
    else:
        projects = json.loads(find_yml_files(YML_PATH), encoding='utf-8')
...
def build():
    ...
    dic = dict(no_cache=json["no_cache"] if "no_cache" in json else None,
               pull=json["pull"] if "pull" in json else None)
